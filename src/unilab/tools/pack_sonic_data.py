...
def _pair_metadata(
    ...
    if robot_frames != smpl_frames or robot_frames < _HISTORY:
...
def _continuity_segments(
    ...
    *,
    ...
    for start, end in zip(boundaries[:-1], boundaries[1:], strict=True):
        start_i = int(start)
        end_i = int(end)
        if end_i - start_i < _HISTORY:
            dropped_frames += end_i - start_i
        else:
            segments.append((start_i, end_i))
    if not segments:
        raise ValueError(
            f"SONIC NPZ pair {robot_path.stem!r} has no continuous segment of {_HISTORY} frames"
        )
...
def pack_sonic_dataset(
    ...
    *,
    ...
    if clip_names is not None:
        requested_names = tuple(clip_names)
        if not requested_names:
            raise ValueError("clip_names must contain at least one SONIC clip")
        if len(set(requested_names)) != len(requested_names):
            raise ValueError("clip_names must not contain duplicates")
        pairs_by_name = {
            robot_path.stem: (robot_path, smpl_path) for robot_path, smpl_path in pairs
        }
        missing_names = [name for name in requested_names if name not in pairs_by_name]
        if missing_names:
            raise FileNotFoundError(
                f"Requested SONIC clips were not found as paired NPZ files: {missing_names}"
            )
        pairs = [pairs_by_name[name] for name in requested_names]

    packed_clip_names: list[str] = []
    ...
    for index, (robot_path, smpl_path) in enumerate(pairs, start=1):
        ...
        for segment_index, (start, end) in enumerate(segments):
            ...
            packed_clip_names.append(segment_name)
    ...
    try:
        ...
        (temporary / "clip_names.txt").write_text(
            "".join(f"{name}\n" for name in packed_clip_names), encoding="utf-8"
        )
        manifest = {
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="clip_names",
        action="append",
        help="include one paired NPZ clip stem; repeat to preserve an explicit clip order",
    )
